@extends('cms.layout')
@section('title', 'عرض مصاريف المدرسة')
@section('title2', 'عرض مصاريف المدرسة')
@section('styles')
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="mt-0 header-title">عرض مصاريف المدرسة</h4>
                <p class="text-muted font-14 mb-3">
                    عرض جميع مصاريف المدرسة 
                </p>
                <a href="{{ route('schoolexpenses.create') }}" class="d-flex justify-content-center bg-info text-white p-1 d-block w-25 m-auto">اضافة مصاريف جديدة</a>
                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap">
                    ...
                    <tbody>
                        @foreach ($Schoolexpenses as $Schoolexpense)
                        <tr id="Schoolexpense_{{ $Schoolexpense->id }}">
                            <td>{{ $loop->index + 1 }}</td>
                            <td>{{ $Schoolexpense->amount }}</td>
                            <td>{{ $Schoolexpense->reason }}</td>
                            <td>{{ $Schoolexpense->expense_date }}</td>
                            <td>{{ $Schoolexpense->note }}</td>
                            <td>
                                <a href="{{ route('schoolexpenses.edit', $Schoolexpense->id) }}">
                                    <i class="fas fa-pen"></i></a>
                                <button class="text-danger ms-2 bg-transparent border-0 delete_btn" type="button" onclick="deleteSchoolexpense('{{ $Schoolexpense->id }}')"><i class=" fas fa-trash-alt"></i></button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
...
@section('scripts')
<script>
    function deleteSchoolexpense(id) {
        axios.delete('/cms/schoolexpenses/' + id).then(function(response) {
            document.getElementById(`Schoolexpense_${id}`).remove();
            Swal.fire({
                position: 'center'
                , icon: 'success'
