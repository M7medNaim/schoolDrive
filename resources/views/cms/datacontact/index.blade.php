@extends('cms.layout')
@section('title', 'عرض بيانات الاتصال')
@section('title2', 'عرض بيانات الاتصال')
@section('styles')
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="mt-0 header-title">عرض بيانات الاتصال</h4>
                <p class="text-muted font-14 mb-3">
                    عرض جميع بيانات الاتصال 
                </p>
                <a href="{{ route('datacontacts.create') }}" class="d-flex justify-content-center bg-info text-white p-1 d-block w-25 m-auto">اضافة معلومات جديدة</a>
                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap">
                    ...
                    <tbody>
                        ...
                        <tr id="datacontact_{{ $datacontact->id }}">
                            <td>{{ $loop->index + 1 }}</td>
                            <td>{{ $datacontact->name }}</td>
                            <td>{{ $datacontact->phone }}</td>
                            <td>
                                <a href="{{ route('datacontacts.edit', $datacontact->id) }}">
                                    <i class="fas fa-pen"></i></a>
                                <button class="text-danger ms-2 bg-transparent border-0 delete_btn" type="button" onclick="deletedatacontact('{{ $datacontact->id }}')"><i class=" fas fa-trash-alt"></i></button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
...
<script>
    function deletedatacontact(id) {
        axios.delete('/cms/datacontacts/' + id).then(function(response) {
            document.getElementById(`datacontact_${id}`).remove();
            Swal.fire({
                position: 'center'
                , icon: 'success'
