...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">تعديل درس </h4>
                            ...
                            <select id="student_id" name="student_id" class="form-control" data-toggle="select2"
                                ...
                                @foreach ($students as $student)
                                    <option value="{{ $student->id }}"
                                        {{ $lesson->student_id == $student->id ? 'selected' : '' }}>
                                        {{ $student->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="lesson_date" parsley-trigger="change" required placeholder="تاريخ الدرس"
                            class="form-control" id="lesson_date" value="{{ $lesson->lesson_date }}" />
                    </div>


                    <div class="text-end">
                        <button class="btn btn-primary waves-effect waves-light"
                            onclick="updateLesson('{{ $lesson->id }}')" type="button">تعديل</button>
...
@section('scripts')
    <script>
        function updateLesson(id) {
            ...
            data._method = 'PUT';
            axios.post(`/cms/lessons/${id}`, data)
                .then(function(response) {
                    window.location.href = '/cms/lessons';
