@extends('cms.layout')
@section('title', 'عرض الضرائب الشهرية')
@section('title2', 'عرض الضرائب الشهرية')
@section('styles')
        ...
        <div class="card">
            <div class="card-body">
                ...
                <p class="text-muted font-14 mb-3">
                    عرض جميع الضرائب الشهرية 
                </p>
                <a href="{{ route('monthlytaxes.create') }}" class="d-flex justify-content-center bg-info text-white p-1 d-block w-25 m-auto">اضافة ضرائب شهرية جديدة</a>
                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap">
                    ...
                    <tbody>
                        ...
                        <tr id="monthlytaxe_{{ $monthlytaxe->id }}">
                            <td>{{ $loop->index + 1 }}</td>
                            <td>{{ $monthlytaxe->taxe_day_month }}</td>
                            <td>{{ $monthlytaxe->amount }}</td>
                            <td>{{ $monthlytaxe->taxe_month }}</td>
                            <td>{{ $monthlytaxe->annualtaxe->taxe_year }}</td>
                            <td>
                                <a href="{{ route('monthlytaxes.edit', $monthlytaxe->id) }}">
                                    <i class="fas fa-pen"></i></a>
                                <button class="text-danger ms-2 bg-transparent border-0 delete_btn" type="button" onclick="deletemonthlytaxe('{{ $monthlytaxe->id }}')"><i class=" fas fa-trash-alt"></i></button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
...
@section('scripts')
<script>
    function deletemonthlytaxe(id) {
        axios.delete('/cms/monthlytaxes/' + id).then(function(response) {
                document.getElementById(`monthlytaxe_${id}`).remove();
                Swal.fire({
                    position: 'center',
                    icon: 'success',
