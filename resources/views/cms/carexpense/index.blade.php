@extends('cms.layout')
@section('title', 'عرض مصاريف المركبة')
@section('title2', 'عرض مصاريف المركبة')
@section('styles')
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="mt-0 header-title">عرض مصاريف المركبة</h4>
                <p class="text-muted font-14 mb-3">
                    عرض جميع مصاريف المركبة 
                </p>
                <a href="{{ route('carexpenses.create') }}" class="d-flex justify-content-center bg-info text-white p-1 d-block w-25 m-auto">اضافة مصاريف جديدة</a>
                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap">
                    ...
                    <tbody>
                        ...
                        <tr id="carexpense_{{ $carexpense->id }}">
                            <td>{{ $loop->index + 1 }}</td>
                            <td>{{ $carexpense->car->type_car }}</td>
                            <td>{{ $carexpense->amount }}</td>
                            <td>{{ $carexpense->reason }}</td>
                            <td>{{ $carexpense->expense_date }}</td>
                            <td>
                                <a href="{{ route('carexpenses.edit', $carexpense->id) }}">
                                    <i class="fas fa-pen"></i></a>
                                <button class="text-danger ms-2 bg-transparent border-0 delete_btn" type="button" onclick="deletecarexpense('{{ $carexpense->id }}')"><i class=" fas fa-trash-alt"></i></button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
...
<script>
    function deletecarexpense(id) {
        axios.delete('/cms/carexpenses/' + id).then(function(response) {
            document.getElementById(`carexpense_${id}`).remove();
            Swal.fire({
                position: 'center'
                , icon: 'success'
